...
**발송 실패 알림**: 메일이 누구에게든 닿지 못하면 `OPS_ALERT_EMAIL`로 알림이 갑니다. 받은 사람은
아무 이상을 못 느끼고 못 받은 사람은 알아챌 방법이 없어, 실패는 수신함만으로는 보이지 않기
때문입니다. 세 가지 경우를 알립니다.

| 상황 | 조치 | 재시도 |
|---|---|---|
| 일부 수신자 거부/실패 | 성공한 사람에게 중복이 가지 않도록 발송 완료 처리 | 안 함(수동 안내 필요) |
| 아무에게도 못 감 | `notified_at` 미기록 | 다음 창에서 자동 |
| `alert_recipients`에 활성 수신자 0명 | — | — |

알림 자체가 실패해도 수집은 계속되며 로그에만 남습니다.
...
- `OPS_ALERT_EMAIL` — 발송 실패 알림 수신 주소(쉼표로 여러 명). 미설정 시 `GMAIL_USER` 자신에게 발송.
