...
import { parseKlcaLaw } from "./klca-law";

const raw = readFileSync(new URL("./__fixtures__/klca-law.html", import.meta.url));
const html = iconv.decode(raw, "euc-kr");

describe("parseKlcaLaw", () => {
  const items = parseKlcaLaw(html);
  it("parses multiple rows", () => {
    expect(items.length).toBeGreaterThan(3);
  });
  it("sets board/source and required fields", () => {
    for (const it of items) {
      expect(it.board).toBe("klca-law");
  ...
  it("derives sourceRef from the rNo post id", () => {
    ...
    expect(items[0].sourceRef).toBe("833");
  });
