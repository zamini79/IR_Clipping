...
export function parseKlcaLaw(html: string): CollectedItem[] {
...
export const klcaLawCollector: Collector = {
  board: "klca-law",
  source: "상장회사협의회",
  async collect() {
    const res = await fetch(KLCA_LAW_LIST, { headers: { "User-Agent": "IR-Clipping-Bot/1.0" } });
    if (!res.ok) throw new Error(`KLCA-LAW ${res.status}`);
