...
import { parseFssBodo } from "./fss-bodo";

const html = readFileSync(new URL("./__fixtures__/fss-bodo.html", import.meta.url), "utf8");

describe("parseFssBodo", () => {
  const items = parseFssBodo(html);
  it("parses multiple rows", () => {
    expect(items.length).toBeGreaterThan(3);
  });
  it("sets board/source and required fields", () => {
    for (const it of items) {
      expect(it.board).toBe("fss-bodo");
  ...
  it("derives collectedAt from the row's 작성일자 cell (KST), not the current time", () => {
    ...
    expect(items[0].collectedAt).toBe("2026-07-20T15:00:00.000Z");
  });
