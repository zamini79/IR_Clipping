...
export const FSS_BODO_LIST = `${BASE}/info/searchBodo.do`;
...
export function parseFssBodo(html: string): CollectedItem[] {
  ...
  $("table.tbList tbody tr").each((_, tr) => {
    ...
    const onclick = a.attr("onclick") ?? "";
    const seqMatch = onclick.match(/selectBodo\('(\d+)'\)/);
...
export const fssBodoCollector: Collector = {
  board: "fss-bodo",
  source: "금융감독원",
  async collect() {
